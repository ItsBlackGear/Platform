...
public abstract class Registrar<T> {
    ...
    public <E extends T> E register(String name, E entry) {
        this.entries.put(new ResourceLocation(this.modId, name), entry);
        return entry;
    }
    ...
    public <E extends T> Holder<T> holder(String name, E entry) {
        this.register(name, entry);
        return this.registry.wrapAsHolder(entry);
    }
