...
public class BiomeManagerImpl {
    ...
    public static void bootstrap() {
        ModLoadingContext.get().getActiveContainer().getEventBus().<RegisterEvent>addListener(event -> {
            event.register(NeoForgeRegistries.Keys.BIOME_MODIFIER_SERIALIZERS, entry -> entry.register(Platform.resource("biome_modifier_codec"), codec = MapCodec.unit(PlatformBiomeModifier.INSTANCE)));
            event.register(NeoForgeRegistries.Keys.BIOME_MODIFIERS, entry -> entry.register(Platform.resource("biome_modifier"), PlatformBiomeModifier.INSTANCE));
        });
    }
                    ...
                    if (optional.isPresent()) {
                        ...
                        return Arrays.stream(GenerationStep.Decoration.values())
                            .anyMatch(decoration -> builder.getGenerationSettings()
                                ...
                                .stream()
                                .anyMatch(match -> WorldGenSerialization.serializeAndCompareFeature(match.value(), placedFeature.value())));
                    }
            ...
            getRegistryEntry(Registries.PLACED_FEATURE, feature)
                .ifPresent(placedFeature ->
                    builder.getGenerationSettings()
                        .getFeatures(decoration)
                        .removeIf(match -> WorldGenSerialization.serializeAndCompareFeature(match.value(), placedFeature.value())));
        }
        ...
        public void removeCarver(GenerationStep.Carving carving, ResourceKey<ConfiguredWorldCarver<?>> carver) {
            getRegistryEntry(Registries.CONFIGURED_CARVER, carver)
                .ifPresent(configuredCarver ->
                    builder.getGenerationSettings()
                        .getCarvers(carving)
                        .removeIf(match -> WorldGenSerialization.serializeAndCompareCarver(match.value(), configuredCarver.value())));
        }
    }
}